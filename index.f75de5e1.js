...
document.addEventListener("click", (event)=>{
    ...
    if (!wall.contains(event.target)) return;
    // Підрахунок координат центру павука
    let left = event.clientX - field.left - wall.clientLeft - spider.offsetWidth / 2;
    // eslint-disable-next-line no-shadow
    let top = event.clientY - field.top - wall.clientTop - spider.offsetHeight / 2;
